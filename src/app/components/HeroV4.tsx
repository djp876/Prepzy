...
export function HeroV4() {
  ...
  const [cover, setCover] = useState(3.4);
  useEffect(() => {
    const calc = () => {
    ...
    <div ref={ref} className="pz-v4-track">
      <section className="pz-v4-sticky">
        ...
        <div style={{ position: "relative", zIndex: 2, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "0 20px" }}>
          {/* text block (fades as video expands) */}
          <motion.div style={reduce ? {} : { opacity: textOpacity, y: textY }}>
            ...
            <h1 className="pz-rise" style={{ fontSize: "clamp(36px, 5.6vw, 64px)", fontWeight: 800, lineHeight: 1.04, letterSpacing: "-0.03em", color: "#fff", margin: "18px 0 0" }}>
              Your <span style={{ color: AMBER, fontStyle: "italic" }}>Daily</span>
              <br />
              Learning Companion.
            </h1>
            <p className="pz-rise" style={{ color: "rgba(255,255,255,0.78)", fontSize: "clamp(15px, 1.6vw, 17px)", lineHeight: 1.55, margin: "14px auto 0", maxWidth: 480 }}>
              Video lessons, 24×7 doubt-solving and practice with instant feedback.
            </p>
          </motion.div>

          {/* expanding video (flex item, scales to full-bleed; sticky overflow clips) */}
          <motion.div
            className="pz-v4-video"
            style={
              reduce
                ? { width: "min(520px, 86vw)", aspectRatio: "520 / 322", borderRadius: 22, marginTop: "clamp(26px, 4vw, 46px)" }
                : { width: "min(520px, 86vw)", aspectRatio: "520 / 322", borderRadius: vidRadius, scale: vidScale, transformOrigin: "center center", marginTop: "clamp(26px, 4vw, 46px)" }
            }
          >
        ...
        {/* scroll hint */}
        {!reduce && (
          <motion.span style={{ position: "absolute", bottom: "5%", left: 0, right: 0, textAlign: "center", zIndex: 2, opacity: hintOpacity, color: "rgba(255,255,255,0.6)", fontSize: 12.5, fontWeight: 500 }}>
